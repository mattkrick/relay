...
var CONDITION = RelayConcreteNode.CONDITION,
    ...
    MATCH_FIELD = RelayConcreteNode.MATCH_FIELD,
...
var getStorageKey = RelayStoreUtils.getStorageKey,
    MATCH_FRAGMENT_KEY = RelayStoreUtils.MATCH_FRAGMENT_KEY;
...
function () {
  ...
  _proto._traverseMatch = function _traverseMatch(field, record) {
    ...
    var typeName = RelayModernRecord.getType(linkedRecord);
    var match = field.matchesByType[typeName];

    if (match != null) {
      var operationLoader = this._operationLoader;
      !(operationLoader !== null) ? process.env.NODE_ENV !== "production" ? invariant(false, 'RelayReferenceMarker: Expected an operationLoader to be configured when using `@match`.') : invariant(false) : void 0;
      var operationReference = RelayModernRecord.getValue(linkedRecord, MATCH_FRAGMENT_KEY);
