...
function testDocument(document) {
  ...
  test("can set and set style properties", function (assert) {
    ...
    assert.equal(elemString(elem), "<div style=\"color:red;background:blue;\"></div>");
    assert.end();
  });
  test("can set and get namespaced attributes", function (assert) {
    ...
    assert.equal(elem.getAttributeNS(ns, "myattr"), blankAttributeNS()); // Should work much like get/setAttribute when namespace is null.
  ...
  test("getElement* methods search outside the body", function (assert) {
    ...
    html.className = "bar";
    assert.equal(document.getElementsByClassName("bar")[0], html); // cleanup
  ...
  test("getElement* methods can be passed to map()", function (assert) {
    ...
    assert.deepEqual(["div", "span"].map(document.getElementsByTagName.bind(document)), [[e1], [e2]]);
    e1.id = "1";
    e2.id = "2";
    assert.deepEqual(["1", "2"].map(document.getElementById.bind(document)), [e1, e2]);
    e1.className = "foo";
    e2.className = "bar";
    assert.deepEqual(["foo", "bar"].map(document.getElementsByClassName.bind(document)), [[e1], [e2]]);
    cleanup();
    assert.end();
  });
