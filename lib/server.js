...
function TestServer() {
  ...
  this.port = 30001;
  this.hostname = 'localhost'; // node 8 default keepalive timeout is 5000ms
...
TestServer.prototype.router = function (req, res) {
  ...
  if (p === '/encoding/invalid') {
    res.statusCode = 200;
    res.setHeader('Content-Type', 'text/html');
    res.setHeader('Transfer-Encoding', 'chunked'); // because node v0.12 doesn't have str.repeat
  ...
  if (p === '/error/redirect') {
    res.statusCode = 301; //res.setHeader('Location', '/inspect');
