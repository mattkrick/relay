...
function testDomElement(document) {
  ...
  test("can getElementsByTagName with *", function (assert) {
    ...
    e1.appendChild(e2);
    e2.appendChild(e3); // non-elements should be ignored
  ...
  test("getElement* methods can be passed to map()", function (assert) {
    ...
    assert.deepEqual(["div", "span"].map(container.getElementsByTagName.bind(container)), [[e1], [e2]]);
    e1.className = "foo";
    e2.className = "bar";
    assert.deepEqual(["foo", "bar"].map(container.getElementsByClassName.bind(container)), [[e1], [e2]]);
    cleanup();
    assert.end();
  });
